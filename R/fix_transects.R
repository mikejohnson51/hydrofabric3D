...
fix_braid_transects2 <- function(
  ...
  xs2 <- st_as_sf(xs)
  not_braids2 <- st_as_sf(not_braids)
  braids2 <- st_as_sf(braids)
  ...
  net_intersects2 <- geos::geos_intersects_any(
    xs2,
    not_braids2
  ) 
  ...
  other_xs2 = dplyr::filter(xs2, 
                           !changed,
                           relative_position != "inner")
  plot(other_xs$geometry)
  plot(other_xs2$geometry)
  ...
  unchanged_inners <- dplyr::filter(xs, 
                                    !changed,
                                    relative_position == "inner")
  unchanged_inners2 <- dplyr::filter(xs2, 
                                    !changed,
                                    relative_position == "inner")
  plot(unchanged_inners$geometry)
  plot(unchanged_inners2$geometry)
  # unchanged_inners <- xs[!xs$changed & xs$relative_position == "inner", ]
  
  # remove excess cross sections by keeping ONLY "changed" flowlines
  xs <- dplyr::filter(xs, changed) 
  xs2 <- dplyr::filter(xs2, changed) 
  # xs <- xs[xs$changed, ]
  ...
  inner_intersects2 <- geos::geos_intersects_any(
    unchanged_inners2$geometry,
    xs2$geometry
  )
  ...
  xs2 <- dplyr::bind_rows(
    xs2,
    unchanged_inners2[!inner_intersects2, ]
  )
  ...
  other_intersects2 <- geos::geos_intersects_any(
    other_xs2$geometry,
    xs2$geometry
  )
  plot(xs$geometry)
  ...
  plot(other_xs2$geometry)
  tmp_other <- other_xs2[!other_intersects2, ]
  plot(xs2$geometry, add = F)
  # plot(other_xs2$geometry, col = "red", add = T)
  plot(tmp_other$geometry, col = "green", add = T)
