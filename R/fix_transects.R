...
#'   
#'   # ggplot2::ggplot() +
#'   #   ggplot2::geom_sf(data = unpacked, ggplot2::aes(color = braid_id)) +
#'   #   gghighlight::gghighlight(braid_id == "braid_3")
#'   
...
#'   
#'   # drop_braids <- unique(unlist(strsplit(unique(drops$braid_id), ", ") )) 
#'   # drop_comids <- unique(drops$comid)
#'   # drop_braids <- unique(drops$braid_id)
#'   # drop_braids <- unique(unlist(strsplit(unique(drops$braid_id), ", ") )) 
#'   
#'   # new_braid_ids <- strsplit(x$braid_id, ", ")
#'   # new_braid_ids <- lapply(new_braid_ids, function(vec) 
#'   #                       paste0(vec[(!vec %in% drop_braids)], collapse = ", ")
#'   #                       )
#'   
#'   # new_braid_ids <- gsub(paste(all_ids, collapse = "|"), "", x$braid_id)
#'   # all_ids2 <- stats::setNames(all_ids, as.numeric(gsub("braid_", "",all_ids)))
#'   # unname(all_ids2[order(as.numeric(names(all_ids2)))])
#'   # all_ids2 <- unname(all_ids[order(as.numeric(names(all_ids)))])
#' 
...
fix_braid_transects6 <- function(
  ...
  #  if no braid threshold is given, set it to the max braid length
  # if(is.null(braid_threshold)) {
  
  # (!is.null(braid_threshold) && braid_threshold == "drop_max")
  # braid_threshold = 20000
  ...
  if(rm_intersects) {
    ...
    net <- dplyr::bind_rows(braids, not_braids)
    ...
    transect_lines$braid_id <- ifelse(is.na(transect_lines$braid_id), "no_braid", transect_lines$braid_id)
    # fin4$braid_id <- ifelse(is.na(fin4$braid_id), "no_braid", fin4$braid_id)
    ...
    # fin4 <- fin4[!(lengths(sf::st_intersects(fin4, net)) > 1 & fin4$braid_id == "no_braid"), ]
  }
  
  # select and reorder columns back to original starting positions
  transect_lines <- transect_lines[starting_names]
  # fin4 <- fin4[starting_names]
  
  
  # mapview::mapview(braids, color = "gold") +
  # mapview::mapview(not_braids, color = "dodgerblue") +
  # # mapview::mapview(fin, color = "red") +
  #   # mapview::mapview(fin2, color = "green") +
  #   mapview::mapview(crossed_flines, color = "red")  +
  #   mapview::mapview(crossers, color = "green") +
  #   mapview::mapview(bad_transects, color = "green")
...
fix_braid_transects5 <- function(
  ...
  # net <- net2
  # # transect_lines <- transects
  # transect_lines <- no_fix_transects
  # terminal_id = "terminalpa"
  # braid_threshold = NULL
  # rm_intersects = TRUE
  ...
  # k = 151
  # 
  # ucoms <- unique(braids_simple$braid_id)
  # 
  #   dplyr::filter(braids_simple, braid_id %in% ucoms[[k]])$comid
  #   net %>% 
  #     dplyr::mutate(
  #       comid = as.character(comid),
  #       labs = dplyr::case_when(
  #         comid %in% dplyr::filter(braids_simple, braid_id %in% ucoms[[k]])$comid ~ "yes",
  #         TRUE ~ "no"
  #       )
  #       ) %>% 
  #     ggplot2::ggplot() +
  #     ggplot2::geom_sf(ggplot2::aes(color = labs)) +
  #     gghighlight::gghighlight(labs == "yes")
  # gghighlight::gghighlight(comid %in% dplyr::filter(braids_simple, braid_id %in% ucoms[[k]])$comid)
  ...
  # braids %>% 
  #   # dplyr::filter(braid_id != "no_braid", !is_multibraid) %>% 
  #   dplyr::mutate(
  #     comid = as.character(comid)
  #     # labs = dplyr::case_when(
  #     #   comid %in% dplyr::filter(braids_simple, braid_id %in% ucoms[[k]])$comid ~ "yes",
  ...
  #   ) %>% 
  #   ggplot2::ggplot() +
  #   ggplot2::geom_sf(ggplot2::aes(color = is_multibraid)) +
  #   gghighlight::gghighlight(braid_id != "no_braid") 
  ...
  for(i in 1:nrow(xs)) {
    ...
    # ggplot2::geom_sf(data = components, ggplot2::aes(color = component_id))+
    # gghighlight::gghighlight(component_id %in% unique(components[bid_in_braids, ]$component_id))
    # get_neighbor_braids(x = braids, ids = bids, only_unique = T)
    ...
    #   ggplot2::geom_sf(data = others, color = "red", lwd = 1.5) +
    # # ggplot2::geom_sf(data = others2, color = "green", lwd = 1) +
    #   ggplot2::geom_sf(data = xs[i, ], color = "forestgreen", lwd = 5)
  ...
  for(i in 1:nrow(xs)) {
    ...
    get_geoms_to_cut <- function(x, 
      ...
      if(method == "comid") {
        ...
        # 
        # # remove self comid
        neighbor_comids <- neighbor_comids[neighbor_comids != comid]
        # 
        # Filter to braid flowlines other than self that are within our given braid id or
        # are a nearby neighboring, flowline of a different (but connected) braid
        others <- dplyr::filter(
