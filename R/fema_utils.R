...
#' @param intersect_group_id character, name of a column in flowlines that should be used to check to group the network 
...
extend_transects_to_polygons <- function(transect_lines,   polygons,  flowlines, max_extension_distance, 
  ...
  transect_hy_id_array     <- transect_lines$hy_id
  ...
  for (i in seq_along(transect_hy_id_array)) {
    ...
    # neighbor_transects <- transects_geos[transect_hy_id_array == current_hy_id & transect_cs_id_array != current_cs_id]
    
    # mapview::mapview(sf::st_as_sf(transects_geos[transect_hy_id_array == current_hy_id & transect_cs_id_array != current_cs_id]), color = "red") +
...
is_valid_transect_line <- function(transect_to_check, transect_lines, flowlines) {
  ...
  # transect_lines <- transects_geos[transect_hy_id_array == current_hy_id & transect_cs_id_array != current_cs_id]
  # flowlines <- flowlines_geos[fline_id_array == current_hy_id]
  # ###   ##   ##   ##   ##   ##   ##   ##   ##   ##  
...
get_transect_extension_distances_to_polygons <- function(transect_lines,   polygons,  flines, max_extension_distance) {
  ...
  # TODO: Add left/right extension distancces to transect data
  # TODO: this can ultimately just be the "transect_lines" variable, dont need to make new "transects_with_distances" variable
  transect_lines <- 
    transect_lines %>% 
    dplyr::left_join(
      extensions_by_id,
  ...
  # transect_geoms       <- geos::as_geos_geometry(transect_lines$geom)
  ...
  for (i in seq_along(transect_hy_id_array)) {
    ...
    # initial check to make sure the extended versions of the transects are valid
    use_left_extension  <- is_valid_transect_line(left_extended_trans, transects_geos, flines_geos)
    use_right_extension <- is_valid_transect_line(right_extended_trans, transects_geos, flines_geos)
    # use_both_extensions <- use_left_extension && use_right_extension
  ...
  # transect_lines2 <- transect_lines 
  # dplyr::mutate(
  #   new_cs_lengthm = as.numeric(sf::st_length(geom))
  # ) %>% 
  # dplyr::relocate(hy_id, cs_id, cs_lengthm, new_cs_lengthm)
  ...
  # Update the "transect_lines" with new geos geometries ("geos_list")
  sf::st_geometry(transect_lines) <- sf::st_geometry(sf::st_as_sf(transects_geos))
