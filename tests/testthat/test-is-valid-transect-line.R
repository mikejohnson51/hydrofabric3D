...
testthat::test_that("is_valid_transect_line() - transect line 1 only intersects flowline 1 time and does NOT interesect transects 3 and 4", {
...
testthat::test_that("is_valid_transect_line() - transect line 2 now intersects with transect line 1, NOT VALID", {
  ...
  plot(flowlines$x)
  plot(trans$x[1], col = 'green', lwd = 8, opacity = 0.5, add = T)
  plot(trans$x[c(1, 2, 3, 4)], lwd = 3, col = "red", add = T)
...
# TODO: move each test in here to its own test
testthat::test_that("is_valid_transect_line() - transect line #5 does NOT intersect other transects but it DOES intersect flowline 2 times (NOT VALID)", {
...
# TODO: move each test in here to its own test
testthat::test_that("is_valid_transect_line() - transect line 5 DOES intersects transects 2 and 3 AND it DOES intersect flowline 2 times (NOT VALID) ", {
...
testthat::test_that("is_valid_transect_line() - check all valid pairings of 2 transect lines that only intersect flowline 1 time", { 
  # transect_lines, 
  # polygons, 
  # flowlines, 
  # crosswalk_id = 'hy_id',  
...
testthat::test_that("is_valid_transect_line() - no transect_to_check geometries given and no geometries given for transects to check against, yields FALSE (i.e. NOT VALID transect)", { 
...
testthat::test_that("is_valid_transect_line() - no transect_to_check geometries given but given a set of VALID transects to check against, yields FALSE (i.e. NOT VALID transect)", { 
...
testthat::test_that("is_valid_transect_line() - 3 linestrings w/ 0 intersections (yields FALSE, i.e. is NOT valid)", { 
  ...
  # dplyr::filter(test_geoms, line_type %in% c("transect", "flowline2")) %>% 
  #   .$x %>% 
  #   plot()
  
  # plot(dplyr::filter(test_geoms, line_type == "flowline")$x)
  # plot(trans$x[1], add = T) 
...
testthat::test_that("is_valid_transect_line() - 2 transects that dont touch eachother and go through a flowline only 1 time, there is also 1 more NON CONNECTED flowline (yields TRUE, i.e. IS VALID))", { 
  ...
  # dplyr::filter(test_geoms, line_type %in% c("transect", "flowline", "flowline2")) %>%
  #   .$x %>%
  #   plot()
  ...
  # plot(flowlines$x)
  # plot(trans$x[1], add = T)
  # plot(trans$x[c(1, 3)], add = T)
  
  # no input transect checked AGAINST set of valid transects
  is_valid <- hydrofabric3D:::is_valid_transect_line(
    geos::as_geos_geometry(trans$x[1]),
    geos::as_geos_geometry(trans$x[c(1, 3)]),
    geos::as_geos_geometry(flowlines)
  )
...
testthat::test_that("is_valid_transect_line() - should return FALSE when input is a POINT", {
  test_geoms <- make_flowlines_and_transects_test_data()
...
testthat::test_that("is_valid_transect_line() - should return FALSE when input is a MULTILINESTRING", {
  # test_geoms <- make_flowlines_and_transects_test_data()
  # 
  # trans <- 
  #   test_geoms %>% 
...
testthat::test_that("is_valid_transect_line() - should return FALSE when input is a POLYGON", {
  test_geoms <- make_flowlines_and_transects_test_data()
...
testthat::test_that("is_valid_transect_line() - should handle NULL geometries", {
  test_geoms <- make_flowlines_and_transects_test_data()
...
testthat::test_that("is_valid_transect_line() - should handle empty geometries by returning FALSE", {
...
testthat::test_that("is_valid_transect_line() - check real flowlines ", { 
